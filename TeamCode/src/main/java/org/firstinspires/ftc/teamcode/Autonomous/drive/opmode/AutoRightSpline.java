...
public class AutoRightSpline extends Main {
    ...
    public void runOpMode() throws InterruptedException {
        ...
        Trajectory preload_trajectory_p3 = drive.trajectoryBuilder(new Pose2d((-9.83 + .05) * -1, -14.93 - .03, Math.toRadians(90)))
                .lineToConstantHeading(new Vector2d((-24) * -1, -11.20 + .5))
                .build();

        //BEGINNING IS P1.end() -- add some wiggle
        Trajectory preload_trajectory_p2 = drive.trajectoryBuilder(new Pose2d((-11.94 + .03) * -1, -58.83 - .03, Math.toRadians(90)))
                .lineToConstantHeading(new Vector2d((-9.83) * -1, -14.93))
                .addDisplacementMarker(() -> drive.followTrajectoryAsync(preload_trajectory_p3))
                .build();

        //BEGINS WHERE IT BEGINS
        Trajectory preload_trajectory_p1 = drive.trajectoryBuilder(new Pose2d((-36.00) * -1, -63.37, Math.toRadians(90.00)))
                .lineToConstantHeading(new Vector2d((-11.94) * -1, -58.83))
                .addDisplacementMarker(() -> drive.followTrajectoryAsync(preload_trajectory_p2))
                .build();


        TrajectorySequence preload_trajectory = drive.trajectorySequenceBuilder(new Pose2d((-36.00) * -1, -63.37, Math.toRadians(90.00)))
                .lineToConstantHeading(new Vector2d((-11.94) * -1, -58.83))
                .lineToConstantHeading(new Vector2d((-9.83) * -1, -14.93))
                .lineToConstantHeading(new Vector2d((-23.3) * -1, -11.20 + .5))
        ...
        TrajectorySequence go_stack_preload_trajectory = drive.trajectorySequenceBuilder(preload_trajectory_p3.end())
                .lineToSplineHeading(new Pose2d((-36.18) * -1, -14.58, Math.toRadians(135.00)))
                //.lineToLinearHeading(new Pose2d(-63.97, -12.25, Math.toRadians(180.00)))
                .lineToLinearHeading(new Pose2d((-64.05) * -1, -12.25 - 1, Math.toRadians(180.00)))
                .build();
...
// previous one
        ...
        TrajectorySequence go_low_junction_trajectory = drive.trajectorySequenceBuilder(new Pose2d(-64.00, -12.25, Math.toRadians(180.00)))
                ...
                .splineToLinearHeading(new Pose2d((-64 + 4) * -1, -12.23, Math.toRadians(180)), Math.toRadians(180))
                .lineToLinearHeading(new Pose2d((-48.03 + .07) * -1, -12.25, Math.toRadians(270.00)))
                .build();


        TrajectorySequence go_stack_low_trajectory = drive.trajectorySequenceBuilder(go_low_junction_trajectory.end())
                .lineTo(new Vector2d((-47.88) * -1, -12.16))
                //.lineToSplineHeading(new Pose2d(-63.98, -12.22, Math.toRadians(180.00)))
                .lineToSplineHeading(new Pose2d((-63.54) * -1, -12.22, Math.toRadians(180.00)))
                .build();
        ...
        */
        ...
        TrajectorySequence park_trajectory = drive.trajectorySequenceBuilder(go_low_junction_trajectory.end())
                .lineToConstantHeading(new Vector2d((-12 + parkY) * -1, -12))
                .build();
        ...
        while(opModeIsActive() && !isStopRequested()) {
            ...
            switch (currentState) {
                ...
                case OUTTAKE:
                    ...
                    if (finished_with_cycle) {
                        ...
                        park_trajectory = drive.trajectorySequenceBuilder(go_low_junction_trajectory.end())
                                ...
                                .lineToConstantHeading(new Vector2d((-12 + parkY + 3) * -1, -12 - 1))
                        ...
                        go_stack_low_trajectory = drive.trajectorySequenceBuilder(go_low_junction_trajectory.end())
                                .lineTo(new Vector2d((-47.88) * -1, -12.16))
                                .lineToSplineHeading(new Pose2d((-64.05 - ADDITIVE_FACTOR_X) * -1, -12.23 - ADDITIVE_FACTOR_Y, Math.toRadians(180.00)))
                ...
                case UNSTACK_WHILE_MOVING_UP:
                    ...
                    go_low_junction_trajectory = drive.trajectorySequenceBuilder(new Pose2d((-64.00) * -1, -12.25, Math.toRadians(180.00)))
                            //.lineToConstantHeading(new Vector2d(-64 + 4, -12.23))
                            .splineToLinearHeading(new Pose2d((-64 + 4) * -1, -12.23, Math.toRadians(180)), Math.toRadians(180))
                            //.lineToLinearHeading(new Pose2d(-48.03 - ADDITIVE_FACTOR_X, -12.25 - 1.95 - ADDITIVE_FACTOR_Y, Math.toRadians(270.00)))
                            .lineToLinearHeading(new Pose2d((-48.03 - ADDITIVE_FACTOR_X) * -1, -12.25 - 1.0 - ADDITIVE_FACTOR_Y, Math.toRadians(270.00)))
